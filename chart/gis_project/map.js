...
function init() {
  ...
  const colorScale = d3
    ...
    .domain([3, d3.max(state.taxes, (d) => d.Data_Value)]);
      ...
      div
        .html(
          "State: " +
            ...
            d.properties.NAME +
            "</strong></h3>" +
            "Average Cost per Pack in " +
            newDataYear.get(d.properties.NAME) +
            "<br>" +
            "was " +
            formatNumber(newData.get(d.properties.NAME))
        )
  ...
  const newData2 = new Map(
    dataYear2.map((d) => [d.LocationDesc, d.Data_Value])
  );
  ...
  svg2
    ...
    .on("mouseover", (event, d) => {
      //console.log("tooltip d", d)
      div2.transition().duration(50).style("opacity", 1);
      div2
        .html(
          "State: " +
            "<strong><h3>" +
            ...
            "<br>" +
            "was " +
            formatNumber(newData2.get(d.properties.NAME))
        )
        .style("left", event.pageX + 10 + "px")
        .style("top", event.pageY - 28 + "px");
    })
  ...
  svg2
    ...
    .attr("y", "87%")
    .attr("x", function (d, i) {
      return width2 - width2 / 2.5 - 20 + i * 30;
    })
  ...
  svg2
    ...
    .attr("y", "93%")
    .attr("x", function (d, i) {
      return width2 - width2 / 2.5 + 5 + i * 30;
    })
    .style("fill", "black")
    .text((d) => d)
  ...
  svg3
    ...
    .on("mouseover", (event, d) => {
      // console.log("tooltip d", d)
      div3.transition().duration(50).style("opacity", 1);
      div3
        .html(
          "Income: " +
            "<h3><strong>" +
    ...
    .on("mouseout", () => {
      div3.transition().duration(100).style("opacity", 0);
    });
  svg3
    .selectAll("text")
    .data(state.income)
